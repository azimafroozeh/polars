...
pub(super) async fn calculate_row_group_pred_pushdown_skip_mask(
    ...
    // Statistics evaluation is CPU-bound over the in-memory footer, so it runs on the compute
    // runtime. Bloom probing stays out of this task; see the probe below.
    let stats_metadata = metadata.clone();
    let stats_row_group_slice = row_group_slice.clone();
    let statistics_mask = executor::spawn(TaskPriority::High, async move {
        let metadata = stats_metadata;
        let row_groups_slice = &metadata.row_groups[stats_row_group_slice.clone()];

        if let Some(ri) = &mut row_index {
            for md in metadata.row_groups[0..stats_row_group_slice.start].iter() {
                ri.offset = ri
                    .offset
    ...
    // Bloom probing issues byte-source range reads (object_store/reqwest create tokio timers at
    // poll time), so it must run on this tokio thread, not the compute executor above.
